...
CELLS = [
    ...
    (
        ...
        "**Expected dry-run insight** (metrics are **computed** from task completion, consistency, and security — not hardcoded):\n\n"
        "```\n"
        "SINGLE AGENT         →  fastest setup, lowest task completion (one generalist)\n"
        "SEQUENTIAL           →  full domain depth, higher latency (dependency order)\n"
        "PARALLEL             →  lowest latency, good completion (parallel waves)\n"
        "PARALLEL + REVIEWER  →  review pass + rework → highest quality (enterprise pick)\n"
        "```"
    ),
    (
    ...
    (
        ...
        "    ├── prompts.py                   ← Per-agent Gemini prompts (live mode)\n"
    ...
    (
        ...
        "| Mode | Flag | API key? |\n"
        "|------|------|----------|\n"
        "| **Dry-run** | `--dry-run` | No — simulated agents, **$0** |\n"
        "| **Live** | `--live` | Yes — Gemini call **per agent** (use `--live-limit`) |"
    ),
    (
        "## 6. Execution Flow\n\n"
        "```\n"
    ...
    (
        ...
        "| `python series-2.8/app.py --strategy reviewer --dry-run` | Reviewer strategy | No |\n"
        "| `python series-2.8/app.py --request-id r0024 --dry-run` | Inspect one request | No |\n"
        "| `python series-2.8/app.py --requests 50 --dry-run` | Faster dev test | No |\n"
        "| `python series-2.8/app.py --live --request-id r0024` | Live — Gemini per agent | Yes |\n"
        "| `python series-2.8/app.py --live --live-limit 2` | Live benchmark (limited) | Yes |"
    ),
    (
        "## 8. Key Code Snippets\n\n"
        ...
        "```\n\n"
        "### Orchestrator + live agents\n\n"
        "```python\n"
        "STRATEGIES = (\"single\", \"sequential\", \"parallel\", \"reviewer\")\n"
        "\n"
        "def orchestrate(request, strategy, *, dry_run=True):\n"
        "    # dry_run=False → run_agent() calls Gemini for each specialist\n"
        "    ...\n"
        "\n"
        "def run_agent(agent_id, request, memory_snapshot, *, dry_run=True):\n"
        "    # Live: generate(build_agent_prompt(...))\n"
        "    # Dry-run: simulate_agent_output(...)\n"
        "    ...\n"
        "```"
    ),
    (
